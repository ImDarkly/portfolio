...
function getImageUrl(source: Project["image"]) {
  return imageUrlBuilder(sanityClient).image(source).width(1600).height(900).url()
}
  ...
  return (
    <main className="flex min-h-svh items-center justify-center px-4 py-6 sm:px-6">
      <div className="flex w-full max-w-2xl flex-col gap-4 text-left">
        ...
        <p className="font-sans text-sm lowercase text-muted-foreground">loading project</p>
      </div>
    </main>
  )
}
      ...
      <div className="flex w-full max-w-2xl flex-col gap-6 text-left">
        <div className="flex flex-col gap-3">
          ...
          <button
            type="button"
            onClick={() => navigate("/")}
            className="inline-flex items-center gap-2 self-start border border-border bg-transparent px-4 py-2 font-heading text-xs lowercase text-muted-foreground transition-colors hover:text-foreground"
          >
    ...
    <main className="flex min-h-svh items-center justify-center px-4 py-6 sm:px-6">
      <div className="flex w-full max-w-2xl flex-col gap-5 text-left">
        ...
        <div className="flex flex-col gap-5">
          <h1 className="break-words font-heading text-4xl font-medium leading-none text-foreground sm:text-6xl">
            {project.title}
          </h1>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {project.techStack.map((tech) => (
                <span
                  ...
                  className="border border-border px-2 py-1 font-heading text-[11px] lowercase text-foreground"
                >
                  {tech}
                </span>
              ))}
            ...
            <div className="flex flex-wrap gap-3 pt-1">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 border border-[#181717] bg-[#181717] px-4 py-2 font-heading text-xs lowercase text-white transition-colors hover:bg-[#181717]/90"
              >
                github
              ...
              {project.liveUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 border border-emerald-500 bg-emerald-500 px-4 py-2 font-heading text-xs lowercase text-background transition-colors hover:bg-emerald-500/90"
                >
                  live demo
                  <RiArrowRightLine className="size-3.5" />
                </a>
